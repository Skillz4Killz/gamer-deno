import { botCache, cache, addRole } from "../../deps.ts";
...
botCache.monitors.set('autorole', {
    ...
    if (message.type !== 0 || !message.member || message.member.roles.length) return
    ...
    addRole(message.guildID, message.author.id, message.author.bot ? settings.botsAutoRoleID : settings.userAutoRoleID)
  }
})