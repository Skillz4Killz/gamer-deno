import { botCache, cache, botHasChannelPermissions, Role, getMessage, Channel, addReaction } from "../../../../deps.ts";
...
createSubcommand('roles-reactions', {
	...
	execute: async function (message, args: CommandArgs, guild) {
		...
		const messageToUse =
			cache.messages.get(args.messageID) || (await getMessage(channel.id, args.messageID).catch(() => undefined))
		if (!messageToUse) return botCache.helpers.reactError(message);

		const reactionRole = await db.reactionroles.findOne((value) => value.messageID === args.messageID || (message.guildID === value.guildID && value.name === args.name));
