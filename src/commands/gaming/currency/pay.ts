...
createCommand({
    ...
    execute: async function (message, args: CommandArgs) {
        ...
        if (amountReceivedToday && amountReceivedToday > 1000 && !botCache.vipUserIDs.has(args.member.id) && botCache.vipGuildIDs.has(message.guildID)) {
            args.amount = 1000;
        }
        ...
        if (!settings || settings.coins < args.amount) return botCache.helpers.reactError(message);

        // Transfer the coins
        db.users.update(message.author.id, { coins: settings.coins - args.amount });
        db.users.update(args.member.id, { coins: (targetSettings?.coins || 0) + args.amount });
