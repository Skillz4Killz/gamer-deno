...
createSubcommand('events-positions', {
    ...
    execute: async function (message, args: CommandArgs) {
        ...
        if (args.amount > event.maxAttendees) return botCache.helpers.reactError(message);
        if (args.amount < 1) args.amount = 1;

        // Make sure this position does not already exists
        if (event.positions.some(p => p.name === args.name)) return botCache.helpers.reactError(message);

        db.events.update(event.id, { positions: [...event.positions, { name: args.name, amount: args.amount || 1 }]});
