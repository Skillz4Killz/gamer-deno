...
createSubcommand('events-positions', {
    ...
    execute: async function (message, args: CommandArgs) {
        ...
        event.acceptedUsers = event.acceptedUsers.map(user => ({ ...user, position: event.positions[0]?.name || "" }));

        // Remove all users 
        db.events.update(event.id, { positions: event.positions, acceptedUsers: event.acceptedUsers })
