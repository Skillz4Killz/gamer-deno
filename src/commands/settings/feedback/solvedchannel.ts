...
createSubcommand("settings-feedback", {
  ...
  execute: async (message, args: SettingsFeedbackSolvedchannelArgs) => {
		// A channel in the same guild was provided
		if (args.channel) {
			db.guilds.update(message.guildID, { solvedChannelID: args.channel.id });
			return botCache.helpers.reactSuccess(message);
		}

		// No channel nor id was provided so disable
		if (!args.channelID) {
			db.guilds.update(message.guildID, { solvedChannelID: "" });
			return botCache.helpers.reactSuccess(message);
		}
		...
		if (!botCache.vipGuildIDs.has(channel.guildID)) return botCache.helpers.reactError(message, true);
		...
		db.guilds.update(message.guildID, { solvedChannelID: args.channelID });
		return botCache.helpers.reactSuccess(message);
  },
});

interface SettingsFeedbackSolvedchannelArgs {
	channel?: Channel;
	channelID?: string;
}
