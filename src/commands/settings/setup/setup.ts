...
import { editMessage, createGuildChannel, botID, OverwriteType, sendMessage, addReactions, botCache, followChannel } from "../../../../deps.ts";
...
createCommand({
    ...
    execute: async function (message, args, guild) {
        ...
        sendMessage(idleChannel.id, `${mention}, **${parsePrefix(message.guildID)}idle create** https://gamer.mod.land/docs/idle.html`);
        ...
        // Step 6: Mails
        await botCache.commands.get('settings')?.subcommands?.get('mails')?.subcommands?.get('setup')?.execute?.(message, {}, guild);
        ...
        // Step 8: URL Filter
        await botCache.commands.get('settings')?.subcommands?.get('automod')?.subcommands?.get('links')?.subcommands?.get("enable")?.execute?.(message, {}, guild);
        await editMessage(loading, createProgressBar(9, 15));

        // Step 9: Profanity Filter
        await botCache.commands.get('settings')?.subcommands?.get('automod')?.subcommands?.get('profanity')?.subcommands?.get("setup")?.execute?.(message, {}, guild);
        await editMessage(loading, createProgressBar(10, 15));

        // Step 10: Capital Filter
        // @ts-ignore
        await botCache.commands.get('settings')?.subcommands?.get('automod')?.subcommands?.get('capitals')?.subcommands?.get("setup")?.execute?.(message, { enabled: true }, guild);
        await editMessage(loading, createProgressBar(11, 15));

        // Step 11: Feedback
        await botCache.commands.get('settings')?.subcommands?.get('feedback')?.subcommands?.get('setup')?.execute?.(message, {}, guild);
        ...
        // Step 14: Mute
        await botCache.commands.get('settings')?.subcommands?.get('mute')?.execute?.(message, {}, guild);
        ...
        await botCache.commands.get('roles')?.subcommands?.get('reactions')?.subcommands?.get('setup')?.execute?.(hold, {}, guild);
