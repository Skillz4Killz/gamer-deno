import {
  botCache,
  Channel,
  botHasChannelPermissions, hasChannelPermissions, Permissions, editMember
} from "../../../deps.ts";
...
createCommand({
  ...
  execute: async function (message, args: MoveArgs, guild) {
    if (!botHasChannelPermissions(args.channel.id, [Permissions.MOVE_MEMBERS]) || !hasChannelPermissions(args.channel.id, message.author.id, [Permissions.MOVE_MEMBERS])) return botCache.helpers.reactError(message);
      // If a valid new channel was provided we simply move all the users in the channel over
      if (args.new) {
        if (!botHasChannelPermissions(args.new.id, [Permissions.MOVE_MEMBERS])) return botCache.helpers.reactError(message);
