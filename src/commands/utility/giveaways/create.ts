...
createSubcommand('giveaway', {
  ...
  if (args.channel && args.title) {
    const embed = new Embed()
      ...
      .setDescription(
        [
          translate(message.guildID, 'strings:GIVEAWAY_CREATE_REACT_WITH', { emoji: botCache.constants.emojis.giveaway }),
          translate(message.guildID, 'strings:GIVEAWAY_CREATE_AMOUNT_WINNERS', { amount: args.winners })
        ].join('\n')
      )
      ...
      const giveawayMessage = await sendEmbed(args.channel.id, embed, translate(message.guildID, 'strings:GIVEAWAY_CREATE_SIMPLE_CONTENT'))?.catch(console.log)
    ...
    return sendMessage(
      message.channelID,
      translate(message.guildID, 'strings:GIVEAWAY_CREATE_CREATED_SIMPLE', { id: message.id, channel: args.channel.mention })
    )
  }
  ...
  sendMessage(message.channelID, translate(message.guildID, 'strings:GIVEAWAY_CREATE_NEED_GIVEAWAY_CHANNEL')).catch(console.error).catch(console.error)
  ...
  sendMessage(message.channelID, translate(message.guildID, 'strings:GIVEAWAY_CREATE_NEED_GIVEAWAY_MESSAGE_ID')).catch(console.error).catch(console.error)
  ...
  if (messageResponse.content !== 'skip' && !requestedMessage)
    sendMessage(message.channelID, translate(message.guildID, 'strings:GIVEAWAY_CREATE_INVALID_MESSAGE', { channel: `<#${channel.id}>` })).catch(console.error).catch(console.error)
  if (messageResponse.content === 'skip')
    sendMessage(message.channelID, translate(message.guildID, 'strings:GIVEAWAY_CREATE_DEFAULT_MESSAGE')).catch(console.error).catch(console.error)

  // The amount of gamer coins needed to enter.

  sendMessage(message.channelID, translate(message.guildID, 'strings:GIVEAWAY_CREATE_NEED_COST_TO_JOIN')).catch(console.error).catch(console.error)
  ...
  if (costResponse.content === 'skip') sendMessage(message.channelID, translate(message.guildID, 'strings:GIVEAWAY_CREATE_DEFAULT_COST'))

  // The role ids that are required to join. User must have atleast 1.

  sendMessage(message.channelID, translate(message.guildID, 'strings:GIVEAWAY_CREATE_NEED_REQUIRED_ROLES_TO_JOIN')).catch(console.error).catch(console.error)
  ...
  if (requiredRolesResponse.content === 'skip')
    sendMessage(message.channelID, translate(message.guildID, 'strings:GIVEAWAY_CREATE_DEFAULT_REQUIRED_ROLES')).catch(console.error).catch(console.error)
  const requiredRoles = requiredRolesResponse.content.split(' ').map(id => parseRole(id)?.id)

  // How long is this giveaway going to last for.

  sendMessage(message.channelID, translate(message.guildID, 'strings:GIVEAWAY_CREATE_NEED_DURATION')).catch(console.error).catch(console.error)
  ...
  if (!duration) sendMessage(message.channelID, translate(message.guildID, 'strings:GIVEAWAY_CREATE_DEFAULT_DURATION'))

  // The amount of winners for this giveaway

  sendMessage(message.channelID, translate(message.guildID, 'strings:GIVEAWAY_CREATE_NEED_AMOUNT_WINNERS')).catch(console.error)
  ...
  if (amountResponse.content === 'skip' || !amount)
    sendMessage(message.channelID, translate(message.guildID, 'strings:GIVEAWAY_CREATE_INVALID_AMOUNT_WINNERS')).catch(console.error)

  // Whether users are allowed to enter the giveaway multiple times.

  sendMessage(message.channelID, translate(message.guildID, 'strings:GIVEAWAY_CREATE_NEED_DUPLICATES')).catch(console.error)
  ...
  const YES_OPTIONS = translate(message.guildID, 'strings:YES_OPTIONS', { returnObjects: true })
  ...
  if (allowDuplicates) {
    sendMessage(message.channelID, translate(message.guildID, 'strings:GIVEAWAY_CREATE_NEED_DUPLICATE_DURATION')).catch(console.error)
    ...
    if (!duplicateCooldown)
      sendMessage(message.channelID, translate(message.guildID, 'strings:GIVEAWAY_CREATE_DEFAULT_DUPLICATE_DURATION')).catch(console.error)
  }

  let emoji = botCache.constants.emojis.giveaway

  if (messageResponse.content !== 'skip') {
    sendMessage(message.channelID, translate(message.guildID, 'strings:GIVEAWAY_CREATE_NEED_EMOJI')).catch(console.error)
    ...
    if (emojiResponse.content === 'skip')
      sendMessage(
        message.channelID,
        translate(message.guildID, 'strings:GIVEAWAY_CREATE_DEFAULT_EMOJI', { emoji })
      )
  ...
  sendMessage(message.channelID, translate(message.guildID, 'strings:GIVEAWAY_CREATE_NEED_PICK_WINNERS')).catch(console.error)
  ...
  sendMessage(message.channelID, translate(message.guildID, 'strings:GIVEAWAY_CREATE_NEED_PICK_INTERVAL')).catch(console.error)
  ...
  if (!pickInterval) sendMessage(message.channelID, translate(message.guildID, 'strings:GIVEAWAY_CREATE_DEFAULT_PICK_INTERVAL'))

  // The channel id where messages will be sent when reaction based like X has joined the giveaway.
  sendMessage(message.channelID, translate(message.guildID, 'strings:GIVEAWAY_CREATE_NEED_NOTIFICATIONS_CHANNEL')).catch(console.error)
  ...
  if (!notificationsChannel) return sendMessage(message.channelID, translate(message.guildID, 'strings:GIVEAWAY_CREATE_INVALID_CHANNEL'))

  // The amount of milliseconds to wait before starting this giveaway.
  sendMessage(message.channelID, translate(message.guildID, 'strings:GIVEAWAY_CREATE_NEED_DELAY_TILL_START')).catch(console.error)
  ...
  if (!duplicateCooldown) sendMessage(message.channelID, translate(message.guildID, 'strings:GIVEAWAY_CREATE_DEFAULT_DELAY_TILL_START'))

  // Whether the giveaway allows entry using commands.

  sendMessage(message.channelID, translate(message.guildID, 'strings:GIVEAWAY_CREATE_NEED_ALLOW_COMMANDS')).catch(console.error)
  ...
  if (allowCommandEntry) {
    ...
    sendMessage(message.channelID, translate(message.guildID, 'strings:GIVEAWAY_CREATE_NEED_SET_ROLES')).catch(console.error)
    ...
    setRoleIDs = setRolesResponse.content.split(' ').map(id => parseRole(id)?.id || '')
  }

  let allowReactionEntry = false
  if (requestedMessage) {
    // Whether the giveaway allows entry using reaction entries.
    sendMessage(message.channelID, translate(message.guildID, 'strings:GIVEAWAY_CREATE_NEED_ALLOW_REACTIONS')).catch(console.error)
  ...
  if (requestedMessage) {
    addReaction(requestedMessage.channelID, requestedMessage.id, emoji).catch(console.log)
  }
