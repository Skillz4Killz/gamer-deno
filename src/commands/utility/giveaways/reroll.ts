...
createSubcommand("giveaway", {
    ...
    execute: async function (message, args) {
        ...
        // Filter out the user
        await db.giveaways.update(args.giveawayID, {         participants: giveaway.participants.filter(p => p.memberID !== args.memberID), pickedParticipants: giveaway.pickedParticipants.filter(p => p.memberID !== args.memberID)
        });
