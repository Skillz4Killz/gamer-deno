...
createCommand({
	...
	execute: async function (message, args, guild) {
		...
		if (!settings || ![settings.ideaChannelID, settings.bugsChannelID].includes(args.channel.id)) return botCache.helpers.reactError(message);
		...
		const messages = await getMessages(args.channel.id, { limit: 100, after: args.messageID });
  ...
  for (const msg of messages || []) {
	...
	sendResponse(message, { file: { blob: new Blob(csvArray), name: "output.csv"} });
	}
})
