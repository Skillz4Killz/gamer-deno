...
createSubcommand('embed', {
  ...
  const channel = botCache.vipGuildIDs.has(message.guildID) && args.channel ? args.channel : guild?.channels.get(message.channelID);
  if (!channel) return botCache.helpers.reactError(message);

  const messageToUse = cache.messages.get(args.messageID) || await getMessage(channel.id, args.messageID).catch(() => undefined);
  if (!messageToUse || messageToUse.author.id !== botID) return botCache.helpers.reactError(message);
  ...
  try {
		...
		sendAlertResponse(message, `https://discord.com/channels/${message.guildID}/${messageToUse.channelID}/${messageToUse.id}`);
  } catch (error) {
