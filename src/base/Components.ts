import { ActionRow, ButtonStyles, MessageComponentTypes, SelectMenuComponent, SelectOption, TextStyles } from "@discordeno/bot";
...
export class Components extends Array<ActionRow> {
    ...
    addButton(
        ...
        if (row.components.length === 5 || row.components.some((component) => component.type !== MessageComponentTypes.Button)) {
            ...
            if (row.components.some((component) => component.type !== MessageComponentTypes.Button)) return this;
        }

        row.components.push({
            type: MessageComponentTypes.Button,
            label: label,
            customId: style !== "Link" && style !== ButtonStyles.Link ? customIdOrLink : undefined,
            style: typeof style === "string" ? ButtonStyles[style] : style,
            emoji: this.stringToEmoji(options?.emoji),
            url: style === "Link" || style === ButtonStyles.Link ? customIdOrLink : undefined,
...
class SelectMenuBuilder {
    ...
    addOption(label: string, value: string, options?: Pick<SelectOption, "description" | "emoji" | "default">) {
    ...
    ok() {
        this.components.__addSelectMenu({ ...this, type: MessageComponentTypes.SelectMenu });
