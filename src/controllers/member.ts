import { controllers, DiscordPayload, GuildMemberAddPayload, cache, structures, botCache, GuildBanPayload } from "../../deps.ts";

controllers.GUILD_MEMBER_ADD = async function handleInternalGuildMemberAdd(data: DiscordPayload) {
    ...
    botCache.eventHandlers.memberAdd?.(guild, payload.user, cache.members.get(payload.user.id));
  }
  
controllers.GUILD_MEMBER_REMOVE = async function handleInternalGuildMemberRemove(data: DiscordPayload) {
