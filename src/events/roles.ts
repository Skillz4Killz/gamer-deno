import { botCache, rawAvatarURL, getAuditLogs, Role, Guild } from "../../deps.ts";
...
async function handleServerLog(guild:Guild, role: Role, type: "created" | "deleted") {
    const texts = [
        translate(guild.id, type === "created" ? "strings:ROLE_CREATED" : "strings:ROLE_DELETED", { name: `<@&${role.id}>`, id: role.id }),
        translate(guild.id, "strings:TOTAL_ROLES", { amount: guild.roles.size }),
        translate(guild.id, "strings:MENTIONABLE", { value: botCache.helpers.booleanEmoji(role.mentionable) }),
        translate(guild.id, "strings:HOISTED", { value: botCache.helpers.booleanEmoji(role.hoist) }),
  ...
  const embed = new Embed()
    ...
    if (botCache.vipGuildIDs.has(guild.id)) {
        if ((type === "created" && logs.roleCreatePublic) || (type === "deleted" && logs.roleDeletePublic)) sendEmbed(logs.publicChannelID, embed)?.catch(console.error)
    }

    const auditlogs = await getAuditLogs(guild.id, { action_type: type === "created" ? "ROLE_CREATE" : "ROLE_DELETE" }).catch(console.error);
    const relevant = auditlogs?.audit_log_entries?.find(e => e.target_id === role.id);
    if (!relevant) return sendEmbed(type === "created" ? logs.roleCreateChannelID : logs.roleDeleteChannelID, embed)?.catch(console.error);
