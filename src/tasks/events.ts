import { botCache, deleteMessage, deleteMessageByID, getMessage, sendMessage, cache, guildIconURL, sendDirectMessage } from "../../deps.ts";
...
botCache.tasks.set('events', {
    ...
    execute: async function () {
        ...
        for (const event of events) {
            ...
            else if (event.startsAt < now && !event.hasStarted && event.endsAt > now) eventsToStart.push(event)
            else if (event.startsAt > now && !event.hasStarted) eventsToRemind.push(event)
        }
...
async function endEvent(event: EventsSchema) {
    ...
    if (!event.isRecurring) {
        ...
        if (event.cardMessageID && event.cardChannelID) deleteMessageByID(event.cardChannelID, event.cardMessageID).catch(console.error);
    ...
    const cardMessage = cache.messages.get(event.cardMessageID) || await getMessage(event.cardChannelID, event.cardMessageID).catch(() => undefined)
    if (!cardMessage) return;

    // If it existed, update it with new info
    botCache.commands.get('events')?.subcommands?.get("card")?.execute?.(cardMessage, { eventID: event.eventID });
}
...
async function startEvent(event: EventsSchema) {
    const embed = new Embed()
      ...
      .addField(translate(event.guildID, `strings:EVENTS_SHOW_RSVP_EMOJI`), `${event.acceptedUsers.length} / ${event.maxAttendees}`)
    ...
    event.acceptedUsers.forEach(user => sendDirectMessage(user.id, { embed }).catch(console.log))
    // Mark the event as has started
    db.events.update(event.id, { hasStarted: true });
    // Send a reminder message to the channel
    const reminder = await sendMessage(event.cardChannelID, { content: botCache.vipGuildIDs.has(event.guildID) ? event.alertRoleIDs.map(id => `<@&${id}>`).join(' ') : "", embed });
    // Delete it after a minute
    if (reminder) deleteMessage(reminder, undefined, botCache.constants.milliseconds.MINUTE).catch(() => undefined);
}
