      ...
      const giveaways = await db.giveaways.findMany({}, true);
          ...
          giveaways.forEach(giveaway => {
            ...
            if (giveaway.hasEnded) {
              ...
              if (endsAt + botCache.constants.milliseconds.DAY > now) return db.giveaways.delete(giveaway.id);
...
export async function pickGiveawayWinners(giveaway:GiveawaySchema) {
  ...
  if (botCache.vipGuildIDs.has(giveaway.guildID)) {
    ...
    filteredParticipants = filteredParticipants.filter(participant => cache.members.get(participant.memberID)?.guilds.has(giveaway.guildID))
  }
  ...
  if (!giveaway.pickWinners && filteredParticipants.length <= giveaway.amountOfWinners) {
    for (const participant of filteredParticipants) {
      ...
      sendEmbed(giveaway.notificationsChannelID, embed, `<@${participant.memberID}>`)
  ...
  await db.giveaways.update(giveaway.id, { pickedParticipants: [...giveaway.pickedParticipants, randomParticipant] });
  ...
  if (giveaway.pickWinners)
    sendEmbed(giveaway.notificationsChannelID, embed, `<@${randomParticipant.memberID}>`)
  else {
    embed
      .setTitle(`Lost the giveaway!`)
      .setDescription(`<@${randomParticipant.memberID}> has lost the giveaway!`)
    sendEmbed(giveaway.notificationsChannelID, embed)
  }

  // If VIP guild enabled the interval option, delay it for that time period
    setTimeout(() => pickGiveawayWinners(giveaway), botCache.vipGuildIDs.has(giveaway.guildID) && giveaway.pickInterval ? giveaway.pickInterval : 1000)
}