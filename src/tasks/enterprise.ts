...
botCache.tasks.set("enterprise", {
  ...
  execute: async function () {
    // PUBLIC BOT SKIP
    if (botId === 270010330782892032n) return;

    // ENTERPRISE SERVERS FOR THIS BOT
    const enterprise = await db.enterprise.get(botId.toString());
    if (!enterprise) return;

    // LEAVE ANY THAT ARENT PART OF THIS PACKAGE
    cache.guilds.forEach((guild) => {
      // KEEP SUPPORT SERVER IN CASE WE NEED
      if (guild.id.toString() === botCache.constants.botSupportServerID) return;
